...
class DichromatTrans {
	...
	/**
	* XYZ配列からProtan変換済みXYZ配列を算出するメソッド
	* @param {math.matrix} inXYZ - XYZ値の配列
	* @param {string} inType - 変換したい色覚タイプ
	* @returns {math.matrix} XYZ値
	*/
	transXYZtoDichroXYZ(inXYZ,inType){
		...
		if(inType = "Protan"){
			afterLMS = this.transLMStoProtan(LMS); 
		}
		else if(inType = "Deutan"){
			afterLMS = this.transLMStoDeutan(LMS); 
		}

		return this.transLMStoXYZ(afterLMS);
	}
